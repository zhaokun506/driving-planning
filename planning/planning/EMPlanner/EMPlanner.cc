...
void EMPlanner::Plan(const TrajectoryPoint &planning_init_point,
                     ...
                     PerceptionObstacle obstacles,
                     Trajectory *ptr_computed_trajectory)
{
    // 1.计算规划起点
    CalPlaningStartPoint();
    std::shared_ptr<PathTimeGraph> sl_graph = std::make_shared<PathTimeGraph>(reference_line, planning_init_point);
    LocalizationInfo localization_info = localization.localization_info();
    sl_graph->set_sl_host(localization_info);

    auto static_obstacles = obstacles.obstacles();
    sl_graph->set_sl_static_obstacles(static_obstacles);
    sl_graph->CreateSamplingPoint();
