...
/// @brief
/// @param emplaner_conf
SpeedTimeGraph::SpeedTimeGraph(ReferenceLine planning_path, EMPlannerConfig emplaner_conf) //传入规划好的路径
{
...
{
    std::vector<ReferencePoint> planning_path_points = planning_path.reference_points();
    ...
    {
        ds2start = ds2start + sqrt(pow(planning_path_points[i].x - planning_path_points[i - 1].x, 2) +
...
// 2.计算障碍物的ST位置
void SpeedTimeGraph::SetDynamicObstaclesSL(const std::vector<ObstacleInfo> dynamic_obstacles)
{
    ...
    ReferenceLineProvider::FindMatchAndProjectPoint(planning_path_, obstacles_xy, 0, 3, match_points, project_points);
    ...
    PathTimeGraph::Cartesian2Frenet(planning_path_, sl_planning_path_, traj_points, match_points, project_points, sl_dynamic_obstacles);
    sl_dynamic_obstacles_ = sl_dynamic_obstacles;
}

void SpeedTimeGraph::GenerateSTGraph(const std::vector<SLPoint> sl_dynamic_obstacles)
{
    //该函数生成st图，使用斜直线模型
    for (const auto &obs : sl_dynamic_obstacles)
    {
        ...
        if (abs(obs.dl_dt) < 0.3)
        {
            ...
            else
            {
                ...
                */
                ...
                double t_collision = (plan_start_.s - obs.ds_dt) / (plan_start_.ds_dt - obs.ds_dt); //碰撞发生的时间
                double s_collision = plan_start_.s + t_collision * plan_start_.ds_dt;               //碰撞发生的位置
                ...
                PathTimeGraph::Frenet2Cartesian(planning_path_, sl_planning_path_, sl_virtual_obstacles_, xy_virtual_obstacles_);
                continue;
            }
        }
...
// 3.采样
void SpeedTimeGraph::CreateSmaplePoint(int row, int col)
{ // row=40,col=16;
...
void SpeedTimeGraph::SpeedDynamicPlanning()
{
    ...
    for (int i = 0; i < row; i++)
    {
        ...
        sample_points_[i][0].ds_dt = sample_points_[i][0].s / sample_points_[i][0].t;
        sample_points_[i][0].dds_dt = sample_points_[i][0].ds_dt - plan_start_.ds_dt / sample_points_[i][0].t;

        sample_points_[i][0].cost2start = CalcDpCost(plan_start_, sample_points_[i][0]);
    ...
    for (int j = 1; j < col; j++)
    {
        for (int i = 0; i < row; i++)
        {
            ...
            for (int k = 0; k < row; k++)
            {
                ...
                sample_points_[k][j].ds_dt = (sample_points_[k][j].s - sample_points_[k][j - 1].s) /
                                             (sample_points_[k][j].t - sample_points_[k][j - 1].t);
                sample_points_[k][j].dds_dt = (sample_points_[k][j].ds_dt - sample_points_[k][j - 1].ds_dt) /
                                              (sample_points_[k][j].t - sample_points_[k][j - 1].t);

                cost = sample_points_[k][j - 1].cost2start + CalcDpCost(sample_points_[i][j], sample_points_[k][j - 1]);
                if (cost < sample_points_[i][j].cost2start)
                {
                    sample_points_[i][j].cost2start = cost;
                    //保留最优的点的ds_dt
                    sample_points_[i][j].ds_dt = (sample_points_[i][j].s - sample_points_[k][j - 1].s) /
    ...
    dp_speed_points.insert(dp_speed_points.begin(), plan_start_); //插入规划起点
    dp_speed_points_ = dp_speed_points;
}
double SpeedTimeGraph::CalcDpCost(STPoint point_s, STPoint point_e)
{ /*
   % 该函数将计算链接两个节点之间边的代价
     % 输入：边的起点的行列号row_start,col_start 边的终点行列号row_end,col_end
 ...
 */
    ...
    double cur_s_dot = (point_e.s - point_s.s) / (point_e.t - point_s.t);                  //计算速度
    double cur_s_dot2 = (point_e.ds_dt - point_s.ds_dt) / (point_e.ds_dt - point_s.ds_dt); //计算加速度
    //计算推荐速度代价
    double cost_ref_speed = emplaner_conf_.speed_dp_cost_ref_speed * pow(cur_s_dot - emplaner_conf_.ref_speed, 2);
    ...
    if (cur_s_dot2 < 4 && cur_s_dot2 > -6)
        ...
        // % 超过车辆动力学限制，代价会增大很多倍
        cost_accel = 100000 * emplaner_conf_.speed_dp_cost_accel * pow(cur_s_dot2, 2);
    double cost_obs = CalcObsCost(point_s, point_e);

    return cost_ref_speed + cost_accel + cost_obs;
}

double SpeedTimeGraph::CalcObsCost(STPoint point_s, STPoint point_e)
{
    /*
    % 该函数将计算边的障碍物代价
    ...
    */
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        { //计算路径点到障碍物的距离，点到线的距离。
            ...
            double dis3 = abs(vector1(1) * vector3(2) - vector1(2) * vector3(1)) / sqrt(vector3.transpose() * vector3);
            if ((vector1.transpose() * vector3 > 0 && vector2.transpose() * vector3 > 0) ||
                (vector1.transpose() * vector3 < 0 && vector2.transpose() * vector3 < 0))
                min_dis = min(dis1, dis2);
            else
                min_dis = dis3;
        }
        cost_obs = cost_obs + CalcCollisionCost(emplaner_conf_.speed_dp_cost_obs, min_dis);
    }
}
...
void SpeedTimeGraph::GenerateCovexSpace()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            if (cur_s > sl_planning_path_[cur_index].s &&cur_s < = sl_planning_path_[cur_index + 1].s)
                break;
        }
        planning_path_points_ = planning_path_.reference_points();
        double k = (planning_path_points_[cur_index + 1].kappa - planning_path_points_[cur_index].kappa) /
                   (planning_path_points_[cur_index + 1].s - planning_path_points_[cur_index].s);
        double cur_kappa = planning_path_points_[cur_index].kappa + k * (cur_s - planning_path_points_[cur_index].s);

        double max_speed = sqrt(emplaner_conf_.max_lateral_accel / (abs(cur_kappa) + 1e-10));
        double min_speed = 0;
    ...
    for (const auto &obs : st_obstacles)
    {
        ...
        // 计算障碍物的纵向速度
        double obs_speed = (obs.right_point.s - obs.left_point.s) / (obs.right_point.t - obs.left_point.t);
        // % 插值找到当t = obs_t时，dp_speed_s 的值
        double mid_index = FindDpMatchIndex(obs_t);
        double min_k = (dp_speed_points_[mid_index + 1].s - dp_speed_points_[mid_index].s) /
                       (dp_speed_points_[mid_index + 1].t - dp_speed_points_[mid_index].t);
        double dp_s = dp_speed_points_[mid_index].s + k * (dp_speed_points_[mid_index + 1].t - dp_speed_points_[mid_index].t);
        ...
        //         break;
        //     }
        //     else if (obs.left_point.t >= dp_speed_points_[j].t && obs.left_point.t < dp_speed_points_[j + 1].t)
        ...
        //         break;
        //     }
        //     else if (obs.right_point.t >= dp_speed_points_[j].t && obs.right_point.t < dp_speed_points_[j + 1].t)
        //     { //% 否则遍历dp_speed_t 找到与obs_st_t_in_set(i)最近的点的编号
        //         t_ub_index = j;
        //         break;
        //     }
        ...
        //% 最低为3 因为碰瓷没法处理
        t_ub_index = min(t_ub_index + 2, dp_end_index);
        ...
        if (obs_s > dp_s) //决策为减速避让
        {
            for (int i = t_lb_index; i < t_ub_index; i++)
            {
                ...
                convex_s_ub_[i] = min(convex_s_ub_[i], obs.left_point.s + obs_speed * (dp_t + obs.left_point.t));
            }
        }
        else
        {
            for (int i = t_lb_index; i < t_ub_index; i++)
            {
                ...
                convex_s_lb_[i] = max(convex_s_ub_[i], obs.left_point.s + obs_speed * (dp_t + obs.left_point.t));
            }
        }
    }
}
...
// 5.二次规划
void SpeedTimeGraph::SpeedQuadraticProgramming()
{
    /*
            %速度二次规划
    ...
    */
    ...
    int n = dp_speed_points_; //??输出
    //初始化输出
    Eigen::MatrixXd qp_s = Eigen::MatrixXd::Ones(n, 1) * nan;
    Eigen::MatrixXd qp_ds_dt = Eigen::MatrixXd::Ones(n, 1) * nan;
    Eigen::MatrixXd qp_dds_dt = Eigen::MatrixXd::Ones(n, 1) * nan;
    Eigen::MatrixXd relative_time = Eigen::MatrixXd::Ones(n, 1) * nan;

    int qp_size = dp_speed_points_.size();
    // Hissen矩阵 3n*3n
    Eigen::MatrixXd A_ref = Eigen::MatrixXd::Zeros(3 * qp_size, 3 * qp_size);
    Eigen::MatrixXd A_dds_dt = Eigen::MatrixXd::Zeros(3 * qp_size, 3 * qp_size);
    Eigen::MatrixXd A_jerk = Eigen::MatrixXd::Zeros(3 * qp_size, qp_size - 1);
    Eigen::MatrixXd H = Eigen::MatrixXd::Zeros(3 * qp_size, 3 * qp_size);

    // f矩阵 3n*1
    Eigen::MatrixXd f = Eigen::MatrixXd::Zeros(3 * qp_size, 1);

    //不等式约束
    Eigen::MatrixXd A = Eigen::MatrixXd::Zeros(qp_size - 1, 3 * qp_size);
    Eigen::MatrixXd b = Eigen::MatrixXd::Zeros(qp_size - 1, 1);

    //等式约束
    Eigen::MatrixXd Aeq = Eigen::MatrixXd::Zeros(2 * qp_size - 2, 3 * qp_size);
    Eigen::MatrixXd Aeq_tran = Eigen::MatrixXd::Zeros(3 * qp_size, 2 * qp_size - 2);
    Eigen::MatrixXd beq = Eigen::MatrixXd::Zeros(2 * qp_size - 2, 1);

    //上下边界约束
    Eigen::MatrixXd A_lu = Eigen::MatrixXd::Identity(3 * qp_size, 3 * qp_size);
    Eigen::MatrixXd lb = Eigen::MatrixXd::Zero(3 * qp_size, 1);
    Eigen::MatrixXd ub = Eigen::MatrixXd::Zero(3 * qp_size, 1);

    Eigen::MatrixXd A_merge = Eigen::MatrixXd::Zero(3 * qp_size + qp_size - 1 + 3 * qp_size, 1);

    Eigen::MatrixXd lb_merge = Eigen::MatrixXd::Zero(3 * qp_size + qp_size - 1 + 3 * qp_size, 1);
    Eigen::MatrixXd ub_merge = Eigen::MatrixXd::Zero(3 * qp_size + qp_size - 1 + 3 * qp_size, 1);
    //生成H
    for (int i = 0; i < qp_size; i++)
    {
        A_ref(3 * i, 3 * i) = 1;
        A_dds_dt(3 * i + 1, 3 * i + 1) = 1;
    }

    Eigen::MatrixXd A_jerk_sub = Zero(6, 1);
    A_jerk_sub << 0,
        0,
        1,
        0,
    ...
    {
        A_jerk_sub.block(3 * i, i, 6, 1) = A_jerk_sub;
    }
    ...
    for (int i = 0; i < qp_size; i++)
    {
        f(3 * i, 3 * i) = 2 * emplaner_conf_.speed_qp_cost_v_ref * emplaner_conf_.ref_speed;
    }
    ...
    Eigen::MatrixXd A_sub = Eigen::MatrixXd A_sub(6, 2);
    A_sub << 1, 0,
        dt, 1,
        (1 / 3) * pow(dt, 2), (1 / 2) * dt,
        -1, 0,
    ...
    for (int i = 0; i < qp_size - 1; i++)
    {
        Aeq_tran.block(3 * i, 2 * i, 2, 6) = A_sub;
    }
    Aeq = Aeq_tran.transpose();
    ...
    A_merge.block(qp_size - 1 + 3 * qp_size, 0, 3 * qp_size, 3 * qp_size) = A_lu;

    lb_merge.block(0, 0, qp_size - 1, 1) = Eigen::MatrixXd::Ones(qp_size - 1, 1) * -DBL_MAX;
    ...
    solver.data()->setNumberOfConstraints(3 * qp_size + qp_size - 1 + 3 * qp_size); // A矩阵行数
...
void SpeedTimeGraph::SpeedQpInterpolation(int n) //点的个数401
{
    ...
    */
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        qp_speed_points_dense_[i].s = qp_speed_points_[j].s + qp_speed_points_[j].ds_dt * dt_2pre +
                                      (1 / 3) * qp_speed_points_[j].dds_dt * pow(dt_2pre, 2) +
                                      (1 / 6) * qp_speed_points_[j + 1].dds_dt * pow(dt_2pre, 2);
        dp_speed_points_dense_[i].ds_dt = qp_speed_points_[j].ds_dt + 0.5 * qp_speed_points_[j].dds_dt * dt_2pre;
        dp_speed_points_dense_[i].dds_dt = qp_speed_points_[j].dds_dt + (qp_speed_points_[j + 1].dds_dt - qp_speed_points_[j].dds_dt) *
...
void SpeedTimeGraph::PathAndSpeedMerge(int n, double cur_t)
{
    ...
    */
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (sl_planning_path_[j] <= qp_speed_points_[i].s && sl_planning_path_[j + 1].s > qp_speed_points_[i].s)
                break;
        }
        //线性插值
        double k = (qp_speed_points_[i].s - sl_planning_path_[j].s) / (sl_planning_path_[j + 1].s - sl_planning_path_[j].s);
        trajectory_points_[i].x = planning_path_[j].x + k * (planning_path_[j + 1].x - planning_path_[j].x);
        trajectory_points_[i].y = planning_path_[j].y + k * (planning_path_[j + 1].y - planning_path_[j].y);
        trajectory_points_[i].heading = planning_path_[j].heading + k * (planning_path_[j + 1].heading - planning_path_[j].heading);
        trajectory_points_[i].kappa = planning_path_[j].kappa + k * (planning_path_[j + 1].kappa - planning_path_[j].kappa);
    }

    trajectory_points_[n - 1].x = planning_path_[planning_path_.size() - 1].x;
    trajectory_points_[n - 1].y = planning_path_[planning_path_.size() - 1].y;
    trajectory_points_[n - 1].heading = planning_path_[planning_path_.size() - 1].heading;
    trajectory_points_[n - 1].kappa = planning_path_[planning_path_.size() - 1].kappa;
    trajectory_points_[n - 1].v = qp_speed_points_[qp_speed_points_.size() - 1].ds_dt;
    trajectory_points_[n - 1].a = qp_speed_points_[qp_speed_points_.size() - 1].dds_dt;

    trajectory_.set_trajectory_points(trajectory_points_);
}

const Trajectory SpeedTimeGraph::trajectory() const{
    return trajectory_;
}
