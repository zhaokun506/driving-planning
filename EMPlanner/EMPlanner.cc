...
void EMPlanner::Plan(const u_int64_t current_time,
  ...
  sl_graph_ = std::make_unique<PathTimeGraph>(reference_line,
                                              planning_init_point, config_);

  sl_graph_->SetHostSl(localization_info);

  auto static_obstacles = obstacles;

  sl_graph_->SetStaticObstaclesSl(static_obstacles);

  sl_graph_->CreateSamplingPoint(11, 4, 15, 1); // 11行，4列
...
void EMPlanner::CalPlaningStartPoint(const Trajectory &pre_traj,
  ...
  //% 误差向量
  auto d_err =
      std::make_pair(local_info.x - pre_x_desire, local_info.y - pre_y_desire);
  //%纵向误差
...
// 2.
void StitchTrajectory(const Trajectory &cur_traj, const Trajectory &stitch_traj,
